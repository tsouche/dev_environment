#!/bin/bash
################################################################################
# Build and Push Rust Dev Container Image on NAS via SSH
...
# Check required files locally first
echo -e "${YELLOW}[Pre-check] Verifying required files...${NC}"
for file in "${REQUIRED_FILES[@]}"; do
    if [ ! -f "${file}" ]; then
        echo -e "${RED}ERROR: Missing required file: ${file}${NC}"
        echo -e "The Dockerfile expects these files for COPY commands."
        echo -e "Please create them or copy from src/env_dev/ if available."
        exit 1
    fi
...
TEMP_DIR="/tmp/docker_build_rustdev_$$"
...
# Transfer Dockerfile
echo -e "${YELLOW}[2/7] Transferring Dockerfile and support files...${NC}"
scp -P ${NAS_PORT} ${DOCKERFILE} ${NAS_USER}@${NAS_HOST}:${TEMP_DIR}/Dockerfile
for file in "${REQUIRED_FILES[@]}"; do
    scp -P ${NAS_PORT} "${file}" ${NAS_USER}@${NAS_HOST}:${TEMP_DIR}/
done
